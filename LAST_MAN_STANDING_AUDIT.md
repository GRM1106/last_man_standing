...
> **Phase P1 remediation note — local, uncommitted, and not deployed.** A new final forward migration replaces global provider-ID uniqueness with `(season, fpl_team_id)` and `(season, fpl_fixture_id)`, adds `football_teams.season`, and replaces `sync_fpl_data` with season-scoped team joins and conflict targets. The migration preserves internal IDs and references and refuses ambiguous/corrupted existing data rather than deduplicating it. Successful-path, rerun, RLS, season-isolation, and intentional-failure rollback tests passed in a disposable local Supabase stack. This foundation still requires independent final review before it is applied anywhere.

> **Phase P2 implementation note — committed and staging schema-verified.** New module 24 adds append-only administrator fixture corrections, raw-versus-effective result selection, mutation-free impact preview, immutable snapshots for newly processed picks, and review-only flagging for corrections affecting processed/completed pots. It is committed on `domain/phase-p2` at `24b183091ec27d9f426c58ab2f1eab787ae7dbe2`, deployed to isolated staging project `last-man-standing-staging` (`evhiixndiuwwodsouyhf`, `eu-west-1`) as migration `20260822002400_result_corrections.sql`, and not merged into `main`. The migration SHA-256 is `7b4af55bf5f3e585fdd2f681cc645339694a7a82cef1e6da2d68d2596a38eccf`; staging contains exactly 24 migrations, introduced no application rows, and passed schema/security inspection. The `pg_dump` self-referencing override-chain foreign-key warning was expected. Staging proves deployment, history, schema shape and security configuration; disposable local testing proves functional behavior, finality, authorization, rollback, idempotency and all three concurrency races. The administrator UI was not live-tested against staging. It intentionally provides no historical replay, snapshot rewriting, winner rewriting, postponement rule, or other P3 behavior. Disposable seed, rollback-verification and concurrency scripts must never run against staging or production. Production application remains prohibited.
