import { createClient } from "@supabase/supabase-js";
import { SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY } from "./config.js";
import {
  addImage,
  addText,
  renderIdentity,
  renderPlayerStandingIdentity,
  renderStandingPick,
  safeImageUrl,
} from "./ui.js";

const supabase = createClient(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY);
const loading = document.querySelector("#dashboard-loading"),
  empty = document.querySelector("#dashboard-empty"),
  content = document.querySelector("#dashboard-content"),
  potsContainer = document.querySelector("#dashboard-pots"),
  greeting = document.querySelector("#dashboard-greeting"),
  potCount = document.querySelector("#pot-count"),
  message = document.querySelector("#dashboard-message"),
  signOutButton = document.querySelector("#sign-out"),
  adminLink = document.querySelector("#admin-link"),
  accountIdentity = document.querySelector("#account-identity"),
  accountName = document.querySelector("#account-name"),
  accountEmail = document.querySelector("#account-email"),
  pickDialog = document.querySelector("#pick-confirmation"),
  pickEmblem = document.querySelector("#pick-confirmation-emblem"),
  pickTitle = document.querySelector("#pick-confirmation-title"),
  pickFixture = document.querySelector("#pick-confirmation-fixture");
const money = (pence) =>
  new Intl.NumberFormat("en-GB", { style: "currency", currency: "GBP" }).format(
    pence / 100,
  );
const paymentCopy = (status) =>
  status === "paid"
    ? ["Paid", "Your payment has been confirmed by the admin."]
    : status === "claimed"
      ? [
          "Awaiting confirmation",
          "You’ve marked this as paid. The admin will confirm it.",
        ]
      : ["Payment due", "Pay the admin, then let them know using the button."];
const titleCase = (value) =>
  value ? value.charAt(0).toUpperCase() + value.slice(1) : "—";

function formDots(team) {
  const form = document.createElement("div");
  form.className = "form-dots";
  (team.form || []).forEach((result) =>
    addText(form, "span", result, `form-dot ${result}`),
  );
  return form;
}
function countdownText(deadline) {
  const remaining = new Date(deadline) - new Date();
  if (remaining <= 0) return "Deadline passed";
  const days = Math.floor(remaining / 86400000),
    hours = Math.floor((remaining % 86400000) / 3600000),
    minutes = Math.floor((remaining % 3600000) / 60000),
    seconds = Math.floor((remaining % 60000) / 1000);
  return `${days ? `${days}d ` : ""}${String(hours).padStart(2, "0")}h ${String(minutes).padStart(2, "0")}m ${String(seconds).padStart(2, "0")}s`;
}
function updateCountdowns() {
  document.querySelectorAll("[data-deadline]").forEach((element) => {
    element.textContent = countdownText(element.dataset.deadline);
    if (new Date(element.dataset.deadline) <= new Date())
      element.closest(".selection-deadline")?.classList.add("passed");
  });
}
setInterval(updateCountdowns, 1000);
function teamButton(team, fixture, pot, selection, isAway = false) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = `pick-team${isAway ? " away" : ""}`;
  addImage(button, team.emblem_url);
  const copy = document.createElement("span");
  copy.className = "pick-team-copy";
  addText(copy, "strong", team.name);
  copy.append(formDots(team));
  if (!team.available) addText(copy, "small", "Already used");
  button.append(copy);
  button.disabled =
    selection.payment_status !== "paid" ||
    selection.player_status !== "active" ||
    selection.deadline_passed ||
    !team.available ||
    new Date(fixture.kickoff_at) <= new Date() ||
    fixture.started ||
    fixture.finished;
  button.addEventListener("click", () =>
    confirmPick(pot, fixture, team, button),
  );
  return button;
}
function renderSelection(pot, selection, panel) {
  panel.replaceChildren();
  const heading = document.createElement("div");
  heading.className = "selection-heading";
  const title = document.createElement("div");
  addText(title, "p", "MAKE YOUR PICK", "label");
  addText(
    title,
    "h3",
    selection.gameweek_number
      ? `Gameweek ${selection.gameweek_number}`
      : "No upcoming gameweek",
  );
  addText(
    heading,
    "span",
    `${selection.remaining_teams?.length || 0} teams remaining`,
    "remaining-count",
  );
  heading.prepend(title);
  panel.append(heading);
  if (selection.deadline) {
    const deadline = document.createElement("div");
    deadline.className = `selection-deadline${selection.deadline_passed ? " passed" : ""}`;
    addText(
      deadline,
      "strong",
      selection.deadline_passed ? "Picks closed" : "Pick deadline",
    );
    const timer = addText(
      deadline,
      "span",
      countdownText(selection.deadline),
      "deadline-countdown",
    );
    timer.dataset.deadline = selection.deadline;
    panel.append(deadline);
    addText(
      panel,
      "p",
      `First kickoff: ${new Date(selection.deadline).toLocaleString("en-GB", { weekday: "long", day: "numeric", month: "long", hour: "2-digit", minute: "2-digit" })}. All picks lock at this time.`,
      "deadline-explanation",
    );
  }
  if (selection.pick) {
    const locked = document.createElement("div");
    locked.className = "locked-pick";
    const image = addImage(locked, selection.pick.emblem_url);
    const copy = document.createElement("div");
    addText(copy, "span", `GW${selection.pick.gameweek_number} PICK`, `label`);
    addText(copy, "h4", selection.pick.team_name);
    addText(
      copy,
      "p",
      `${selection.pick.home_name} v ${selection.pick.away_name} · ${new Date(selection.pick.kickoff_at).toLocaleString("en-GB", { weekday: "short", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}`,
    );
    locked.replaceChildren(image, copy);
    addText(locked, "span", "Locked", "locked-mark");
    panel.append(locked);
    return;
  }
  if (!selection.gameweek_number) {
    addText(
      panel,
      "p",
      "There are no upcoming fixtures in this pot yet.",
      "selection-note",
    );
    return;
  }
  if (selection.payment_status !== "paid")
    addText(
      panel,
      "p",
      "You can review the fixtures now. Team selection unlocks after the admin confirms your entry payment.",
      "selection-note warning",
    );
  const grouped = new Map();
  (selection.fixtures || []).forEach((fixture) => {
    const day = fixture.kickoff_at
      ? new Date(fixture.kickoff_at).toLocaleDateString("en-GB", {
          weekday: "long",
          day: "numeric",
          month: "long",
        })
      : "Date to be confirmed";
    if (!grouped.has(day)) grouped.set(day, []);
    grouped.get(day).push(fixture);
  });
  grouped.forEach((fixtures, day) => {
    const section = document.createElement("section");
    section.className = "fixture-day";
    addText(section, "h4", day, "fixture-day-title");
    fixtures.forEach((fixture) => {
      const row = document.createElement("div");
      row.className = "pick-fixture";
      const centre = document.createElement("div");
      centre.className = "fixture-centre";
      const kickoff = fixture.kickoff_at
        ? new Date(fixture.kickoff_at).toLocaleTimeString("en-GB", {
            hour: "2-digit",
            minute: "2-digit",
          })
        : "TBC";
      addText(
        centre,
        "b",
        fixture.finished
          ? `${fixture.home_score} – ${fixture.away_score}`
          : kickoff,
      );
      addText(
        centre,
        "span",
        fixture.finished
          ? "Full time"
          : fixture.started
            ? "Started"
            : "Kickoff",
      );
      row.append(
        teamButton(fixture.home_team, fixture, pot, selection),
        centre,
        teamButton(fixture.away_team, fixture, pot, selection, true),
      );
      section.append(row);
    });
    panel.append(section);
  });
  const availability = document.createElement("section");
  availability.className = "team-availability";
  addText(availability, "div", "TEAM AVAILABILITY", "availability-title");
  const clubs = document.createElement("div");
  clubs.className = "remaining-clubs";
  (selection.all_teams || []).forEach((team) => {
    const club = document.createElement("span");
    club.className = `remaining-club${team.available ? "" : " used"}`;
    club.title = team.available
      ? `${team.name} is available`
      : `${team.name} has already been used`;
    addImage(club, team.emblem_url);
    club.append(document.createTextNode(team.short_name));
    if (!team.available) addText(club, "small", "Used");
    clubs.append(club);
  });
  availability.append(clubs);
  panel.append(availability);
}
async function loadPotSelection(pot, panel) {
  const [selection, teams] = await Promise.all([
    supabase.rpc("get_pot_selection", { selected_pot_id: pot.id }),
    supabase.rpc("get_my_team_availability", { selected_pot_id: pot.id }),
  ]);
  const firstError = selection.error || teams.error;
  if (firstError) {
    panel.replaceChildren();
    addText(
      panel,
      "p",
      `Team selection isn’t ready: ${firstError.message}. Run 23 — Player team availability list in Supabase.`,
      "selection-note warning",
    );
    return;
  }
  selection.data.all_teams = teams.data;
  if (selection.data.gameweek_number) {
    const deadline = await supabase.rpc("get_gameweek_deadline", {
      selected_pot_id: pot.id,
      selected_gameweek: selection.data.gameweek_number,
    });
    if (deadline.error) {
      panel.replaceChildren();
      addText(
        panel,
        "p",
        `Pick deadline isn’t ready: ${deadline.error.message}. Run 20 — Pick deadlines and random assignment audit in Supabase.`,
        "selection-note warning",
      );
      return;
    }
    Object.assign(selection.data, deadline.data);
  }
  renderSelection(pot, selection.data, panel);
  updateCountdowns();
}
function askToConfirmPick(pot, fixture, team) {
  const kickoff = new Date(fixture.kickoff_at).toLocaleString("en-GB", {
    weekday: "long",
    day: "numeric",
    month: "long",
    hour: "2-digit",
    minute: "2-digit",
  });
  const safeEmblem = safeImageUrl(team.emblem_url);
  if (safeEmblem) pickEmblem.src = safeEmblem;
  else pickEmblem.removeAttribute("src");
  pickTitle.textContent = team.name;
  pickFixture.textContent = `${pot.name} · ${fixture.home_team.name} v ${fixture.away_team.name} · ${kickoff}`;
  pickDialog.showModal();
  return new Promise((resolve) =>
    pickDialog.addEventListener(
      "close",
      () => resolve(pickDialog.returnValue === "confirm"),
      { once: true },
    ),
  );
}
async function confirmPick(pot, fixture, team, button) {
  if (!(await askToConfirmPick(pot, fixture, team))) return;
  button.disabled = true;
  message.textContent = `Locking in ${team.name}…`;
  const { error } = await supabase.rpc("confirm_team_pick", {
    selected_pot_id: pot.id,
    selected_fixture_id: fixture.id,
    selected_team_id: team.id,
  });
  if (error) {
    message.textContent = error.message;
    button.disabled = false;
    return;
  }
  message.textContent = `${team.name} is confirmed and locked.`;
  await loadDashboard();
}
function renderPlayerStandings(data, panel) {
  panel.replaceChildren();
  addText(panel, "h3", "Pot standings");
  addText(
    panel,
    "p",
    "Picks are revealed to everyone once the gameweek deadline passes.",
    "standings-player-note",
  );
  const players = data?.players || [],
    gameweeks = data?.gameweeks || [];
  if (!players.length) {
    addText(panel, "p", "No standings are available yet.", "selection-note");
    return;
  }
  const board = document.createElement("div");
  board.className = "standings-board";
  const table = document.createElement("table");
  table.className = "standings-table player-standings-table";
  const head = document.createElement("thead"),
    headRow = document.createElement("tr"),
    nameHead = document.createElement("th");
  nameHead.className = "standings-player";
  nameHead.textContent = "Player";
  headRow.append(nameHead);
  gameweeks.forEach((gameweek) => {
    const th = document.createElement("th");
    th.textContent = `GW${gameweek}`;
    headRow.append(th);
  });
  head.append(headRow);
  const body = document.createElement("tbody");
  players.forEach((player) => {
    const row = document.createElement("tr"),
      identity = document.createElement("td");
    identity.className = "standings-player";
    renderPlayerStandingIdentity(identity, player);
    row.append(identity);
    const picks = new Map(
      (player.picks || []).map((pick) => [pick.gameweek_number, pick]),
    );
    gameweeks.forEach((gameweek) => {
      const cell = document.createElement("td"),
        pick = picks.get(gameweek);
      cell.className = `standings-cell ${pick?.outcome || "empty"}`;
      if (pick) renderStandingPick(cell, pick);
      else cell.textContent = "—";
      row.append(cell);
    });
    body.append(row);
  });
  table.append(head, body);
  board.append(table);
  panel.append(board);
}
async function loadPlayerStandings(pot, panel) {
  const { data, error } = await supabase.rpc("get_pot_standings", {
    selected_pot_id: pot.id,
  });
  if (error) {
    panel.replaceChildren();
    addText(panel, "h3", "Pot standings");
    addText(
      panel,
      "p",
      `Standings aren’t ready: ${error.message}. Run 21 — Player-visible standings with pick privacy in Supabase.`,
      "selection-note warning",
    );
    return;
  }
  renderPlayerStandings(data, panel);
}
function renderPot(pot) {
  const card = document.createElement("article");
  card.className = "dashboard-pot";
  const header = document.createElement("header");
  header.className = "dashboard-pot-header";
  const title = document.createElement("div");
  addText(title, "p", `${pot.season} · ${titleCase(pot.status)}`, "label");
  addText(title, "h2", pot.name);
  addText(
    title,
    "p",
    `${money(pot.entry_fee_pence)} entry · ${money(pot.buy_back_fee_pence)} one-time buy-back`,
  );
  addText(
    header,
    "span",
    titleCase(pot.player_status),
    `pot-player-status ${pot.player_status}`,
  );
  header.prepend(title);
  const summary = document.createElement("div");
  summary.className = "dashboard-summary compact";
  [
    ["Player status", titleCase(pot.player_status)],
    ["Buy-back", titleCase(pot.buy_back_status)],
  ].forEach(([label, value]) => {
    const item = document.createElement("div");
    item.className = "summary-item";
    addText(item, "span", label);
    addText(item, "strong", value);
    summary.append(item);
  });
  const payment = document.createElement("div");
  payment.className = "payment-panel";
  const paymentText = document.createElement("div");
  const [statusLabel, statusDescription] = paymentCopy(pot.payment_status);
  addText(
    paymentText,
    "strong",
    statusLabel,
    `payment-status ${pot.payment_status}`,
  );
  addText(paymentText, "p", statusDescription);
  payment.append(paymentText);
  if (pot.payment_status === "unpaid") {
    const button = addText(payment, "button", "I’ve paid", "claim-button");
    button.type = "button";
    button.addEventListener("click", () => claimPayment(pot.id, button));
  }
  const buyBack = document.createElement("div");
  buyBack.className = "buy-back-panel";
  if (
    pot.player_status === "eliminated" &&
    pot.buy_back_status === "available"
  ) {
    const copy = document.createElement("div");
    addText(
      copy,
      "strong",
      `Use your ${money(pot.buy_back_fee_pence)} buy-back?`,
    );
    const deadline = pot.buy_back_deadline
      ? new Date(pot.buy_back_deadline).toLocaleString("en-GB", {
          weekday: "long",
          day: "numeric",
          month: "long",
          hour: "2-digit",
          minute: "2-digit",
        })
      : "the next gameweek";
    addText(
      copy,
      "p",
      `Pay the admin and submit your claim before ${deadline}. You can only use this once.`,
    );
    const button = addText(
      buyBack,
      "button",
      "I’ve paid — claim buy-back",
      "claim-button",
    );
    button.type = "button";
    button.addEventListener("click", () => claimBuyBack(pot.id, button));
    buyBack.prepend(copy);
  } else if (pot.buy_back_status === "claimed") {
    addText(buyBack, "strong", "Buy-back awaiting approval");
    addText(
      buyBack,
      "p",
      "Your claim was submitted in time. The admin will confirm your payment and reactivate you.",
    );
  } else if (pot.buy_back_status === "used") {
    addText(buyBack, "strong", "Buy-back used");
    addText(
      buyBack,
      "p",
      "You are back in the pot. Your one-time buy-back has now been used.",
    );
  } else if (pot.buy_back_status === "expired") {
    addText(buyBack, "strong", "Buy-back expired");
    addText(
      buyBack,
      "p",
      "The next gameweek began before a claim was submitted.",
    );
  }
  const gameweeks = document.createElement("section");
  gameweeks.className = "pot-gameweeks";
  addText(gameweeks, "h3", "Included gameweeks");
  const chips = document.createElement("div");
  chips.className = "gameweek-chips";
  const included = pot.gameweeks || [];
  if (included.length) {
    const first = included[0],
      last = included.at(-1),
      label =
        first === last
          ? `GW${first}`
          : `GW${first} → GW${last} · ${included.length} gameweeks`;
    addText(chips, "span", label, "gameweek-chip");
  }
  gameweeks.append(chips);
  const selection = document.createElement("section");
  selection.className = "selection-panel";
  selection.id = `pick-${pot.id}`;
  addText(selection, "p", "Loading fixtures…", "selection-note");
  const history = document.createElement("section");
  history.className = "pick-history";
  history.id = `history-${pot.id}`;
  addText(history, "h3", "Your pick history");
  addText(history, "p", "Loading previous rounds…", "selection-note");
  const standings = document.createElement("section");
  standings.className = "player-standings";
  standings.id = `standings-${pot.id}`;
  addText(standings, "h3", "Pot standings");
  addText(standings, "p", "Loading standings…", "selection-note");
  const quickNav = document.createElement("nav");
  quickNav.className = "pot-quick-nav";
  [
    ["Pick a team", selection.id],
    ["My history", history.id],
    ["Standings", standings.id],
  ].forEach(([label, id]) => {
    const link = document.createElement("a");
    link.href = `#${id}`;
    link.textContent = label;
    quickNav.append(link);
  });
  card.append(header, quickNav, summary, payment);
  if (buyBack.childElementCount) card.append(buyBack);
  card.append(gameweeks, selection, history, standings);
  potsContainer.append(card);
  loadPotSelection(pot, selection);
  loadPickHistory(pot, history);
  loadPlayerStandings(pot, standings);
}
async function loadPickHistory(pot, panel) {
  const { data, error } = await supabase.rpc("get_my_pot_history", {
    selected_pot_id: pot.id,
  });
  panel.replaceChildren();
  addText(panel, "h3", "Your pick history");
  if (error) {
    addText(
      panel,
      "p",
      `Pick history isn’t ready: ${error.message}. Run 18 — Tournament lifecycle, history and test reset in Supabase.`,
      "selection-note warning",
    );
    return;
  }
  if (!data.length) {
    addText(panel, "p", "No completed or locked picks yet.", "selection-note");
    return;
  }
  const list = document.createElement("div");
  list.className = "pick-history-list";
  data.forEach((pick) => {
    const row = document.createElement("div");
    row.className = "pick-history-row";
    const week = document.createElement("strong");
    week.textContent = `GW${pick.gameweek_number}`;
    const image = addImage(row, pick.emblem_url);
    const detail = document.createElement("span");
    addText(detail, "b", pick.team_name);
    addText(
      detail,
      "small",
      `${pick.home_name} v ${pick.away_name} · ${pick.selection_source}`,
    );
    const outcome = document.createElement("em");
    outcome.className = `history-outcome ${pick.outcome}`;
    outcome.textContent = pick.outcome;
    row.replaceChildren(week, image, detail, outcome);
    list.append(row);
  });
  panel.append(list);
}
async function loadDashboard() {
  const { data, error } = await supabase.rpc("get_my_dashboard");
  if (error) {
    loading.querySelector("h1").textContent = "Couldn’t load your dashboard.";
    const detail = addText(loading, "p", error.message, "intro");
    detail.style.fontSize = ".9rem";
    return;
  }
  if (data?.email) {
    accountName.textContent = data.first_name || "Player";
    accountEmail.textContent = data.email;
    accountIdentity.hidden = false;
  }
  if (!data?.approved || !data?.pots?.length) {
    loading.hidden = true;
    empty.hidden = false;
    return;
  }
  loading.hidden = true;
  content.hidden = false;
  greeting.textContent = "Last Man Standing 26/27 Pots";
  potCount.textContent = `${data.pots.length} pot${data.pots.length === 1 ? "" : "s"}`;
  potsContainer.replaceChildren();
  data.pots.forEach(renderPot);
}
async function claimPayment(potId, button) {
  button.disabled = true;
  message.textContent = "Saving your payment claim…";
  const { error } = await supabase.rpc("claim_pot_payment", {
    selected_pot_id: potId,
  });
  if (error) {
    button.disabled = false;
    message.textContent = error.message;
    return;
  }
  message.textContent = "Payment marked as sent. The admin can now confirm it.";
  await loadDashboard();
}
async function claimBuyBack(potId, button) {
  button.disabled = true;
  message.textContent = "Submitting your buy-back claim…";
  const { error } = await supabase.rpc("claim_buy_back", {
    selected_pot_id: potId,
  });
  if (error) {
    button.disabled = false;
    message.textContent = `${error.message}. Run 14 — One-time buy-back workflow in Supabase.`;
    return;
  }
  message.textContent =
    "Buy-back claimed. The admin can now confirm your payment.";
  await loadDashboard();
}
async function initialise() {
  const {
    data: { session },
  } = await supabase.auth.getSession();
  if (!session) {
    window.location.replace("/");
    return;
  }
  const { data: isAdmin } = await supabase.rpc("is_current_user_admin");
  adminLink.hidden = !isAdmin;
  await loadDashboard();
}
signOutButton.addEventListener("click", async () => {
  signOutButton.disabled = true;
  await supabase.auth.signOut();
  window.location.replace("/");
});
initialise();
