...
`npm run test:db` now builds a uniquely isolated disposable database through the complete migration history, including the critical corrective migrations; it never resets the development stack. See [CRITICAL_REMEDIATION.md](CRITICAL_REMEDIATION.md) for prerequisites and boundaries. The Phase 2J verification claims a local run, sends deterministic provider fixtures through the transactional ingestion-and-scan entry point, checks health, and rolls back. The same suite starts two genuine database sessions concurrently and requires one running import plus one recorded overlap. `tests/scheduler-pipeline.test.js` covers fetch/validate/ingest/scan sequencing, partial/malformed rejection, failure isolation, bounded retry, and pre-fetch overlap skipping without the live FPL API.
