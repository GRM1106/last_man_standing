...
async function loadPlayers(){const {data,error}=await supabase.from("profiles").select("id,email,display_name,first_name,last_name,approved,is_admin,created_at").order("created_at",{ascending:false});if(error){message.textContent=`Couldn’t load players: ${error.message}`;return}renderPlayers(data)}
...
async function initialise(){const {data:{session}}=await supabase.auth.getSession();if(!session){window.location.replace("/");return}const {data:isAdmin,error}=await supabase.rpc("is_current_user_admin");if(error){loading.hidden=true;denied.hidden=false;denied.querySelector(".intro").textContent=`Admin check failed: ${error.message}`;return}if(!isAdmin){showDenied();return}loading.hidden=true;content.hidden=false;await loadPlayers()}
