-- 21 — Player-visible standings with pick privacy
-- Run once in the Supabase SQL Editor after pick_deadlines.sql.
...
begin
  ...
  if not viewer_is_admin and not exists(select 1 from public.pot_players where pot_id=selected_pot_id and player_id=(select auth.uid())) then
  ...
  select jsonb_build_object('pot_id',pot.id,'pot_name',pot.name,'season',pot.season,'status',pot.status,
    ...
    'players',coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'email',case when viewer_is_admin then profile.email else null end,
      'player_status',membership.player_status,
      'payment_status',case when viewer_is_admin then membership.payment_status else null end,
      'buy_back_status',case when viewer_is_admin then membership.buy_back_status else null end,
      'picks',coalesce((select jsonb_agg(jsonb_build_object(
        'gameweek_number',pick.gameweek_number,'team_name',team.name,'short_name',team.short_name,'emblem_url',team.emblem_url,
      ...
      where pick.pot_id=pot.id and pick.player_id=membership.player_id and (
        viewer_is_admin
        ...
        or now()>=(select min(fixture.kickoff_at) from public.football_fixtures fixture where fixture.season=pot.season and fixture.gameweek_number=pick.gameweek_number)
      )),'[]'::jsonb)
    ) order by case membership.player_status when 'winner' then 0 when 'active' then 1 else 2 end,
      coalesce(profile.first_name,profile.display_name,profile.email))
    from public.pot_players membership join public.profiles profile on profile.id=membership.player_id
    where membership.pot_id=pot.id),'[]'::jsonb)
